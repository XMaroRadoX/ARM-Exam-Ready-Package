"""Execute the delivered ARMASM objects against independent paper-derived cases.
...
SP=0x201F0000; STOP=0x20000F00; DATA=0x20010000
def run(args):
...
class Machine:
    def __init__(self,path):
        ...
        run([BIN/'armasm.exe','--cpu','Cortex-M3','--diag_suppress=A1950W',path,'-o',obj])
...
def test_machine(m):
    ...
    if 'kruskal' in names:
        ...
        m.cases.append('Kruskal: all nine button-selected increment/offset pairs on3x4; all three output arrays match independent paper model')
        ...
        for path in sources:
            ...
            if key not in cache:
                try:
                    ...
                    cache[key]={'status':'PASS','cases':m.cases,'nativeAssembly':'Arm Compiler6.22 ARMASM object assembled; linked with LLVM for emulation','limits':'Does not simulate hardware exception entry, IRQ timing, peripherals, or the physical board.'}
