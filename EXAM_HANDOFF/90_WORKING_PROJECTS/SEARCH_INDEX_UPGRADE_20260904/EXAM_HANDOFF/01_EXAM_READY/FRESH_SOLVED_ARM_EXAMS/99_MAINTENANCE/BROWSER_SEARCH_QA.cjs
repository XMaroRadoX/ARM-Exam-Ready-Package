...
(async()=>{
 ...
 const search=async(q)=>{await page.goto(url('search.html','?q='+encodeURIComponent(q)));await page.locator('.best-matches .search-result').first().waitFor();};
 await search('digitSum');check((await page.locator('.best-matches .search-result h3 a').first().getAttribute('href')).startsWith('exams/2023-09-18-q1.html'),'Global function result');
 await search('20230918');check((await page.locator('.best-matches .search-result h3 a').first().getAttribute('href')).startsWith('exams/e2023-09-18.html'),'Compact date');
