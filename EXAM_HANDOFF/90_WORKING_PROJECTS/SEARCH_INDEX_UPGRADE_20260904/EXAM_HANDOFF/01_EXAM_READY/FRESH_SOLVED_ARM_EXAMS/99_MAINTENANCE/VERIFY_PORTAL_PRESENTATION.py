...
MANUAL_LINKS = {
    'https://www.keil.com/dd/docs/datashts/arm/cortex_m3/r2p1/dui0552a_cortex_m3_dgug.pdf',
    'https://www.keil.com/support/man/docs/armasm/default.htm',
...
class Page(HTMLParser):
    ...
    def handle_starttag(self, tag, pairs):
        ...
        for name in ('href', 'src', 'action'):
            if attrs.get(name): self.links.append((tag, name, attrs[name], attrs.get('data-external-reference')))
...
def audit():
    ...
    for path, page in pages.items():
        ...
        for tag, attribute, link, reference in page.links:
            links += 1
            url = urlsplit(link)
            # Optional cited manuals are hyperlinks, never remote page dependencies.
            if tag == 'a' and attribute == 'href' and reference == 'manual' and link in MANUAL_LINKS:
                continue
