...
(function(host,factory){const logic=factory();if(typeof module==='object'&&module.exports)module.exports=logic;else host.ARM_PORTAL_LOGIC=logic;})(typeof window==='object'?window:this,function(){
  ...
  function fields(item){const title=equivalent(item.title),heading=equivalent(item.heading),aliases=equivalent([...(item.aliases||[]),names(item.title),...(item.functions||[]).map(names),item.examId,item.date,item.question].join(' ')),topics=equivalent([...(item.topics||[]),...(item.components||[]),...(item.architecture||[])].join(' ')),body=equivalent(item.text||item.summary),functions=(item.functions||[]).map(equivalent);return {title,heading,aliases,topics,body,functions,hay:[title,heading,aliases,topics,body].join(' ')};}
  function rank(item,f,query){const q=equivalent(query),tokens=words(query);if(!q)return 1;if(!tokens.every(w=>f.hay.includes(w)))return 0;let n=f.title===q?10000:f.title.startsWith(q)?1500:f.title.includes(q)?900:0;
    if(f.functions.includes(q))n+=String(item.id).startsWith('question-')?9000:String(item.id).startsWith('exam-')?7000:f.body.includes(q)?7500:0;if(item.examId&&q===equivalent(item.date))n+=String(item.id).startsWith('exam-')?14000:String(item.id).startsWith('question-')?11000:3000;if(f.heading===q)n+=1200;else if(f.heading.includes(q))n+=550;if(f.aliases.includes(q))n+=450;if(f.topics.includes(q))n+=600;if(f.body.includes(q))n+=100;
  ...
  function prepare(input){
    ...
    const items=input.flatMap(i=>[i,...(i.alternateSources||[]).filter(a=>a.id).map(a=>({...a,text:i.text,code:i.code,documentRoute:!/[#]|\.(?:pdf|html?)(?:$|[?#])/i.test(i.route)&&!/[#]|\.(?:pdf|html?)(?:$|[?#])/i.test(a.route)?i.route:a.route,alternateSources:[{title:i.title,route:i.route,sourceClass:i.sourceClass}]}))]);
    ...
    function search(query,filters={}){const tokens=words(query);let candidates=null;
      for(const token of tokens){const set=new Set();for(const word of vocabulary)if(word.includes(token))for(const i of index.get(word))set.add(i);if(!/^[a-z0-9_]+$/.test(token))prepared.forEach((f,i)=>{if(f.hay.includes(token))set.add(i);});candidates=candidates===null?set:new Set([...candidates].filter(i=>set.has(i)));if(!candidates.size)break;}
      const indices=candidates===null?items.map((_,i)=>i):[...candidates];
      return indices.filter(i=>(tokens.length||filters.sourceClass||filters.materialType||filters.track||!String(items[i].id).startsWith('text-'))&&matchesFilters(items[i],filters)).map(i=>({item:items[i],rank:rank(items[i],prepared[i],query)})).filter(x=>x.rank>0).sort((a,b)=>b.rank-a.rank||a.item.title.localeCompare(b.item.title)||a.item.route.localeCompare(b.item.route));
    }
    ...
    function suggestions(query){const q=norm(query);if(!/^[a-z_][a-z0-9_]{3,60}$/.test(q))return [];return vocabulary.filter(v=>v!==q&&Math.abs(v.length-q.length)<=1&&distanceOne(q,v)).slice(0,5);}
    ...
    function study(query,filters={}){
      ...
      const direct=[...catalog.values()].filter(item=>matchesFilters(item,filters)&&(!question||norm(item.question)===question)).map(item=>({item,rank:score(item,query),match:item})).filter(result=>result.rank>0);
      ...
      for(const result of candidates){
        ...
        const directRank=matchesFilters(parent,filters)?score(parent,query):0;
