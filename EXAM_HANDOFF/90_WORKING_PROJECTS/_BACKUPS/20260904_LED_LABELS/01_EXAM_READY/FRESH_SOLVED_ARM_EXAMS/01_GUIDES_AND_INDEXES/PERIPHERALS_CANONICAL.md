...
API indexes 0 through 7 correspond to GPIO bits P2.0 through P2.7 and physical
labels LED11 down to LED4. For a physical label, use index `11 - label`.

```c
exam_led_on(7u);       /* LED4 */
exam_led_one_hot(3u);  /* only LED8; returns EXAM_OK, not a bit mask */
exam_led_write(0x81u); /* LED4 and LED11 */
```

Check the returned status for indexed calls. `exam_led_write()` writes the full
...
for (;;) {
  ...
  previous = current;
  if ((pressed & EXAM_JOY_SELECT) != 0u) exam_led_toggle(0u);
}
```
