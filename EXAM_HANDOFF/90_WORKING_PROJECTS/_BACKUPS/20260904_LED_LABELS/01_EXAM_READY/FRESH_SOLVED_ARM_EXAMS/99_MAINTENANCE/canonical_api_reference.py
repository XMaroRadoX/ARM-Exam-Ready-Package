...
GROUP_DESCRIPTIONS = {
    ...
    "Timers": "Configure and control Timer0 through Timer3 using MR0 and the selected timer's real peripheral clock.",
    ...
    "Joystick": "Read the five active-low joystick controls and derive new press edges.",
...
VALUE_GUIDANCE: dict[str, list[dict[str, object]]] = {
    ...
    "exam_led_on": [value("0, 4, 5, 6, 7", "LED indexes appearing in reviewed button, timer, and result scenarios.", "Observed in reviewed solutions", ("2023-09-18-q2", "2023-02-07-q2"))],
    ...
    "exam_led_toggle": [value("0", "Simple heartbeat or confirmed-button indicator.", "Recommended package recipe")],
    "exam_led_one_hot": [value("0..7", "Moving position or selected item; mask a computed index only when the algorithm guarantees that behavior.", "API range")],
...
int main(void) {
  ...
  for (;;) { __WFI(); }
    ...
    "exam_led_on": d(
        ...
        params={"index": p("in", "GPIO bit index 0..7. Physical LED label = 11 - index: index 0 is LED11 and index 7 is LED4. Other values are rejected without changing the LEDs.")},
        ...
        example="if (exam_led_on(3u) != EXAM_OK) { /* invalid index */ }",
        mistake="Using physical board labels instead of the template's 0..7 professor numbering.",
    ...
    "exam_led_off": d(
        ...
        params={"index": p("in", "LED index 0..7.")},
        returns="EXAM_OK, or EXAM_OUT_OF_RANGE when index is greater than 7.",
    ...
    "exam_led_toggle": d(
        ...
        example="if ((pending & 1u) != 0u) { (void)exam_led_toggle(0u); }",
    ...
    "exam_led_one_hot": d(
        ...
        params={"index": p("in", "Selected LED index 0..7.")},
        returns="EXAM_OK, or EXAM_OUT_OF_RANGE when index is greater than 7.",
        preconditions="Call exam_init() first.",
        side_effects="Replaces the complete eight-bit LED output with 1u << index.",
        context="Bounded call; safe in foreground or a short handler.",
        example="(void)exam_led_one_hot(position & 7u);",
        mistake="Passing a bit mask such as 0x08 instead of the index 3.",
    ...
    "exam_led_write": d(
        ...
        params={"value": p("in", "Bit mask for LEDs 0..7; bit n controls LED n.")},
        returns="No value.",
        preconditions="Call exam_init() first.",
    ...
    "exam_debounce_config": d(
        ...
        params={
            ...
            "confirmation_ms": p("in", "Required stable-press duration in milliseconds; must be greater than 0. Rounded up to whole samples."),
        },
        ...
        exam_note="The API owns the sample count, but your chosen periodic interrupt owns when each tick occurs.",
    ...
    "exam_joystick_read": d(
        ...
        exam_note="For one action per press, compare successive samples with exam_joystick_pressed_edges().",
