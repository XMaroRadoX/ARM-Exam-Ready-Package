...
(() => {
  ...
  function initSearch() {
    ...
    function restore(){
      ...
      allSources=params.get('scope')==='all'||sourceFilters();
      if(advanced)advanced.open=controls.some(c=>c.dataset.searchFilter!=='kind'&&c.value);
    }
    function save(){setUrl(Object.fromEntries([['q',input.value.trim()],['scope',allSources?'all':''],...controls.map(c=>[c.dataset.searchFilter,c.value])]),keys);}
    ...
    function row(result){
      ...
      a.textContent=item.title;h.append(a);
      const excerpt=document.createElement('p');highlight(excerpt,window.ARM_PORTAL_LOGIC.excerpt(result.match||item,input.value),input.value);
      const meta=document.createElement('p');meta.className='result-meta';
      meta.textContent=(allSources?[item.sourceClass||'Maintained',item.kind,item.materialType]:[item.kind,...(item.question?['Question and solution',...(item.languages||[])]:[])]).filter(Boolean).join(' · ');
      article.append(h,excerpt,meta);
      if (allSources && (item.paperRoute || (item.solutionRoutes || []).length)) {
      ...
      for(const related of result.related||[]){const link=document.createElement('a');link.href=related.route;link.textContent=related.kind;const p=document.createElement('p');p.className='result-meta';p.append(link);article.append(p);}
      const others=allSources?[...(result.sections||[]),...(item.alternateSources||[])]:[];
      if(others.length){
    ...
    function render(){
      ...
      active.hidden=!active.textContent;
      if(scope){scope.textContent=allSources?'Back to study pages':'Search all source material';scope.setAttribute('aria-pressed',String(allSources));}
      if(!input.value.trim()&&!controls.some(c=>c.value)){
        count.textContent='';more.hidden=true;
      ...
      for(const result of raw){
        ...
        const route=!allSources?item.id:/\.pdf(?:#|$)/i.test(item.route)?item.route:item.route.split('#')[0];
      ...
      if(!matches.length){
        ...
        matches.slice(0,limit).forEach(r=>results.append(row(r)));
      }
      root.dataset.lastSearchMs=(performance.now()-started).toFixed(1);
    }
    form.addEventListener('submit',event=>{event.preventDefault();clearTimeout(timer);limit=10;save();render();});
    input.addEventListener('input',()=>{clearTimeout(timer);limit=10;timer=setTimeout(render,60);});
    controls.forEach(c=>c.addEventListener('change',()=>{clearTimeout(timer);limit=10;if(sourceFilters())allSources=true;save();render();}));
    const clear=button('Clear filters',()=>{clearTimeout(timer);controls.forEach(c=>{c.value='';});limit=10;save();render();});
    (advanced||root).append(clear);
    if(scope)scope.addEventListener('click',()=>{clearTimeout(timer);allSources=!allSources;if(!allSources)controls.forEach(c=>{if(['sourceClass','materialType','kind'].includes(c.dataset.searchFilter))c.value='';});limit=10;save();render();});
    window.addEventListener('popstate',()=>{clearTimeout(timer);limit=10;restore();render();});
    restore();render();
  }
  function initApi() {
