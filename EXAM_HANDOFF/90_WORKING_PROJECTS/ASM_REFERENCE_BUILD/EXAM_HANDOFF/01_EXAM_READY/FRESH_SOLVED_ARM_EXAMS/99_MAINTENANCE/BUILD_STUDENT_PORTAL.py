...
def build_patterns(rows: list[dict[str, str]], exams: list[dict[str, str]], solutions: list[dict[str, str]], items: list[dict[str, object]]) -> dict[str, Path]:
    ...
    for row in rows:
        ...
        matching_solution = next((solution_by_id[value] for value in related_exam_ids if value in solution_by_id), {})
        c_path = resolve_index_path(row.get("c_source", ""), COURSE)
        asm_path = resolve_index_path(row.get("arm_source", ""), COURSE)
        if c_path and c_path.suffix.lower() in {".md", ".markdown", ".html", ".htm"} and matching_solution:
            c_path = resolve_index_path(matching_solution.get("current_c_source", ""), LIBRARY) or c_path
        if asm_path and asm_path.suffix.lower() in {".md", ".markdown", ".html", ".htm"} and matching_solution:
            asm_path = resolve_index_path(matching_solution.get("current_assembly_source", ""), LIBRARY) or asm_path
        c_code = reference_code(c_path, ("c", "cpp"))
        asm_code = reference_code(asm_path, ("asm", "assembly", "armasm", "s"))
        for label, content, source, language in (("C", c_code, c_path, "C"), ("assembly", asm_code, asm_path, "Assembly")):
            fragment += f"<h2>Actual {label} code</h2>"
            if source:
                broader = any(part in str(source) for part in ("Solved Exams", "exam-solutions", "FRESH_SOLVED")) or source.suffix.lower() in {".md", ".html"}
                if broader:
                    fragment += "<p>This is a broader source reference, not a minimal pattern implementation. Use its exact relevant function and compare the contract before adapting. " + href(source, destination, "Read the related source") + "</p>"
                else:
                    fragment += "<p>Reusable source listing. Check its signature, preconditions, and dependencies before inserting it into the current project.</p>" + details_code(label + " source", content, source, destination, language, True)
            else:
                fragment += "<p>This workflow is explained by the method and trace above; no standalone implementation is claimed.</p>"
...
def main() -> None:
    ...
    expected = {"exams": 23, "questions": 48, "patterns": 78, "api": 52, "peripheral lessons": 14}
