...
static uint32_t pat_alg_edit_distance_001_min3(uint32_t first, uint32_t second,
                                               uint32_t third) {
  uint32_t minimum = (first < second) ? first : second;
  return (third < minimum) ? third : minimum;
}

uint32_t pat_alg_edit_distance_001(const char *first, const char *second,
  ...
  for (row = 1u; row <= first_length; ++row) {
    ...
    for (column = 1u; column <= second_length; ++column) {
      ...
      current[column] =
          pat_alg_edit_distance_001_min3(deletion, insertion, substitution);
    }
    for (column = 0u; column <= second_length; ++column) {
...
uint32_t pat_alg_edit_distance_001_full(const char *first, const char *second,
                                        uint32_t first_length,
