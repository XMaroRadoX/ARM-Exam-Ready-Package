...
def verify():
    ...
    for key,files in PROJECTS.items():
        ...
        for name,expected in files.items():
            ...
            if gcc and name.endswith(".c"):
                includes=[str(p) for p in (folder/"Source").rglob("*") if p.is_dir()]+[str(folder/"Source")]
                cmd=[gcc,"-std=c11","-fsyntax-only","-Werror=implicit-function-declaration",*[v for p in includes for v in ("-I",p)],str(folder/name)]
    ...
    report={"structuralChecks":checks,"errors":errors,"hostSyntaxWithActualHeaders":syntax,
            "nativeKeilBuild":"Not available: Keil uVision/ARMASM and the device pack were not found in checked paths. LLVM target compilation is reported separately.",
