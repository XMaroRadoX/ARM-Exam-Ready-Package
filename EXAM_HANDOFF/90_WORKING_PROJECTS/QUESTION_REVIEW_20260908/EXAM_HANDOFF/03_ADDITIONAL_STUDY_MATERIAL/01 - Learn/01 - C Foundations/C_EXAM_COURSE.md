...
[Official Combined Exam API quick reference](../../../01_EXAM_READY/02_STARTING_TEMPLATES/Official%20Combined%20Exam%20API%20Reference/EXAM_API_QUICK_REFERENCE.md)
...
For a real array in the scope where it is defined, divide its total byte size
by one element's byte size:
...
This is the C equivalent of “array length.” It is not `sizeof values` alone:
that returns bytes, not elements.

An array parameter does not contain its length. Inside
`void f(uint32_t values[])`, `values` is only a pointer, so the same expression
would produce the wrong answer. Calculate the count at the caller and pass
pointer plus count:
