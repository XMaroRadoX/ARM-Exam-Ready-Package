...
An address does not contain an array length. For a callable routine, pass the
base address and element count separately, normally as `R0=base` and
`R1=count`.

When the array is declared in the same ARMASM file, labels can calculate a
compile-time element count:

```asm
values          DCD     4, 7, 9, 2
values_end
VALUES_COUNT    EQU     (values_end - values) / 4
```

This count is `4`; it is not discovered from `R0` at runtime.
