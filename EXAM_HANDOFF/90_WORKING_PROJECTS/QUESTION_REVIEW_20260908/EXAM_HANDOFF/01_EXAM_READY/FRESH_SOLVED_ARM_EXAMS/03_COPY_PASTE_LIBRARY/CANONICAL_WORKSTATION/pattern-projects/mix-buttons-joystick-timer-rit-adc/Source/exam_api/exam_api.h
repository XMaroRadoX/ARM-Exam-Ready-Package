...
#define EXAM_MATCH_INTERRUPT (1u << 0)
#define EXAM_MATCH_RESET     (1u << 1)
#define EXAM_MATCH_STOP      (1u << 2)
...
/* Advanced configuration follows a successful standard configuration.
 * Requires power on and TCR bits 0..1 clear; otherwise EXAM_NOT_READY.
 * Does not start/reset TC. Standard configuration still resets PR to zero.
 * Match: channel 0..3, ticks > 0, actions any combination of EXAM_MATCH_*.
 * Clears only the selected match flag; enables its IRQ without changing priority.
 * Prescaler: any uint32_t PR, clears PC only. Divider: 1, 2, 4, or 8.
 * Invalid arguments leave hardware/software unchanged. */
...
/* Pure tests of one saved exam_timer_ack() result; invalid index returns 0. */
...
/* Same (previous, current) convention; masks to the five valid input bits. */
