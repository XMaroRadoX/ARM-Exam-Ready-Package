...
uint32_t algorithm_algorithm_pat_alg_edit_distance_001_c(const char *first, const char *second,
  ...
  if ((first == NULL) || (second == NULL) || (previous == NULL) ||
      (current == NULL) || (current == previous) || first_length == UINT32_MAX || second_length >= UINT32_MAX/4u) {
    return UINT32_MAX;
  }
  ...
  for (row = 1u; row <= first_length; ++row) {
    ...
    for (column = 1u; column <= second_length; ++column) {
      ...
      current[column] =
          algorithm_algorithm_pat_alg_edit_distance_001_c_min3(deletion, insertion, substitution);
    }
    for (column = 0u; column <= second_length; ++column) {
...
uint32_t algorithm_algorithm_pat_alg_edit_distance_001_c_full(const char *first, const char *second,
  ...
  if ((first == NULL) || (second == NULL) || (table == NULL) ||
