...
def body(data,c_code,asm_code,fixture,c_path,asm_path,test_path,dest,details,href):
    ...
    exam_question = str(data.get('exam_question','')).strip()
    question_html = (f'<div class="question"><h3>Exam question</h3><p>{esc(exam_question)}</p></div>'
                     if exam_question else '')
    mistakes = [str(item) for item in data.get('mistakes',[]) if str(item).strip()]
    mistakes_html = (f'<h3>Common exam mistakes</h3><ul>{"".join(f"<li>{esc(item)}</li>" for item in mistakes)}</ul>'
                     if mistakes else '')
