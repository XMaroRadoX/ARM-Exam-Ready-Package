...
def run(args,cwd):
    # CreateProcess rejects long working directories even when the tools accept
    # absolute input/output filenames. All arguments already resolve absolutely.
    p=subprocess.run([str(a) for a in args],cwd=ROOT.parent,capture_output=True,text=True,timeout=120)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for i,f in enumerate(as_completed(futures),1):
            ...
            report=dict(nativeToolchain='Arm Compiler 6.22 native compiler, ARMASM and linker; complete SW_Debug project source lists; flags match the unchanged uVision baseline',devicePack='Keil LPC1700_DFP 2.7.2',projects=results,physicalBoard='Not tested')
