...
"""An array stores consecutive same-type elements. uint32_t a[3] has indexes 0,1,2. Where a real array is defined, sizeof a / sizeof a[0] gives its element count; sizeof a alone gives bytes. In most expressions an array becomes a pointer to its first element. A pointer carries no length, so calculate the count at the caller and pass it separately to functions. The caller must provide a valid region.
...
'''uint32_t values[] = {2u, 4u, 6u};
uint32_t count = sizeof values / sizeof values[0]; /* 3 */
...
uint32_t total = sum(values, count);
...
"sizeof a / sizeof a[0] inside sum uses the pointer size, not the caller's array size. <=n reads past the end. A wrong stride can appear correct for square matrices; test non-square dimensions.",
