...
"""Translate a loop in four pieces: initialization, entry condition, body, and update. An array address has no runtime length metadata. For a callable routine, pass the base address and element count separately, normally in R0 and R1. If an array is declared in the same assembly file, a label after it can be used to calculate a constant count with (array_end-array)/element_size. Check an empty count before the first load. Keep the invariant visible: at the top, index i identifies the next element, and the accumulator summarizes only earlier elements.
