...
int32_t Maclaurin_cos(int32_t y,uint32_t n){mock_calls++;mock_arg[0]=(uint32_t)y;mock_arg[1]=n;return y;}
...
void EINT1_IRQHandler(void)
{
  ...
  if (exam_timer_config_ticks(EXAM_TIMER1,1592u,
...
EINT1_IRQHandler();CHECK(mock_period[1]==1592);
for(int i=0;i<8978;i++)TIMER1_IRQHandler();CHECK(mock_calls==8978&&repeat_count==200&&mock_arg[1]==3);
TIMER1_IRQHandler();CHECK(mock_dac==0&&!mock_running[1]);EINT1_IRQHandler();CHECK(!mock_running[1]);
CHECK(cosineValues[22]==500);
return 0;}
