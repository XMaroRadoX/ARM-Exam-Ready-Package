...
int Mastermind(int*g,int*s,int*a,int*b){mock_calls++;for(int i=0;i<4;i++)if(a[i]||b[i])mock_arg[6]=1;return (int)mock_return;}
#include <stdint.h>
#include "exam_api.h"
#include "LPC17xx.h"
enum {
...
extern int Mastermind(int guess[4], int secret[4],
...
static int guess[4];
static int secret[4];
static int usedGuess[4];
static int usedSecret[4];
...
{
  int result;
  clear_words(usedGuess);
  clear_words(usedSecret);
  result = Mastermind(guess, secret, usedGuess, usedSecret);
...
static void review_step(void){
    ...
    if ((edges & EXAM_JOY_UP) != 0u) increment_digit(0u);
    if ((edges & EXAM_JOY_RIGHT) != 0u) increment_digit(1u);
    if ((edges & EXAM_JOY_DOWN) != 0u) increment_digit(2u);
    if ((edges & EXAM_JOY_LEFT) != 0u) increment_digit(3u);
    __WFI();
  }
...
for(int j=0;j<2;j++){mock_joy=0;for(int k=0;k<3;k++)RIT_IRQHandler();review_step();mock_joy=EXAM_JOY_UP;for(int k=0;k<3;k++)RIT_IRQHandler();review_step();}
...
mock_joy=0;for(int k=0;k<3;k++)RIT_IRQHandler();review_step();mock_return=0xf0;mock_joy=EXAM_JOY_SELECT|EXAM_JOY_UP;for(int k=0;k<3;k++)RIT_IRQHandler();review_step();CHECK(game_state==FINISHED&&mock_led==0xf0&&guess[0]==0);
mock_joy=0;for(int k=0;k<3;k++)RIT_IRQHandler();review_step();mock_joy=EXAM_JOY_UP;for(int k=0;k<3;k++)RIT_IRQHandler();review_step();CHECK(game_state==FINISHED&&mock_led==0xf0);
return 0;}
