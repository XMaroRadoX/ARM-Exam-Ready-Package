...
uint32_t run_length_encoding(uint32_t v){mock_arg[0]=v;mock_calls++;return 0x1234;}
/* Exam of 3 February 2026: debounced button and stable result display. */
#include <stdint.h>
#include "exam_api.h"
#include "LPC17xx.h"

extern uint32_t run_length_encoding(uint32_t digits);
...
static void review_step(void){
    ...
    if ((exam_button_events_take() & EXAM_BUTTON_EVENT_KEY1) != 0u && have_sample) {
      exam_led_write((uint8_t)run_length_encoding(latest_value));
    }
    __WFI();
  }
...
mock_buttons=2;review_step();CHECK(mock_calls==0);mock_adc_value=0x7b0;mock_adc_fresh=1;review_step();CHECK(mock_led==123);
EINT1_IRQHandler();CHECK(mock_arg[7]==1);mock_buttons=2;review_step();CHECK(mock_arg[0]==123&&mock_led==0x34);
