...
void game_next_round(void)
{
    ...
    /* The tenth LED has already had its FULL 2.5-second window.
    ...
    if (n == 10u) {
        ...
        exam_rit_stop();
        if (hit > miss) {
            exam_led_on(10u);              /* Victory. */
        } else {
            exam_led_on(11u);              /* Defeat, including a tie. */
        }
        return;
    }
    ...
    exam_led_on((uint8_t)(4u + remainder));
...
void TIMER1_IRQHandler(void)
{
    ...
    if ((pending & 1u) != 0u) {            /* MR0 is our 2.5-second interval. */
        game_next_round();
    }
}
...
TIMER1_IRQHandler();CHECK(waiting_for_move==1);mock_joy=expected_direction;RIT_IRQHandler();CHECK(hit==1&&waiting_for_move==0&&mock_led==0);
mock_joy=0;RIT_IRQHandler();mock_joy=EXAM_JOY_UP|EXAM_JOY_DOWN;RIT_IRQHandler();CHECK(miss==0);
for(int i=1;i<10;i++){TIMER1_IRQHandler();mock_joy=0;RIT_IRQHandler();mock_joy=EXAM_JOY_UP|EXAM_JOY_DOWN;RIT_IRQHandler();}
CHECK(mock_calls==10&&miss==9);TIMER1_IRQHandler();CHECK(mock_led==(1u<<(11-11))&&!mock_running[1]&&!mock_rit_running);
mock_joy=0;RIT_IRQHandler();mock_joy=EXAM_JOY_LEFT;RIT_IRQHandler();CHECK(mock_led==(1u<<(11-11))&&mock_calls==10);
return 0;}
