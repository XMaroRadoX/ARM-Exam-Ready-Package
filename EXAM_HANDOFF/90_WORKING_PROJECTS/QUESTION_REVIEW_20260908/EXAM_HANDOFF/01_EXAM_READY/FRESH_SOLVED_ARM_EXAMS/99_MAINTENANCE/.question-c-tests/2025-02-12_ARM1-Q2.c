...
int32_t Maclaurin(int32_t y,uint32_t n){mock_calls++;mock_arg[0]=(uint32_t)y;mock_arg[1]=n;return y;}
...
EINT0_IRQHandler();CHECK(mock_period[0]==1263);
for(int i=0;i<8978;i++)TIMER0_IRQHandler();CHECK(mock_calls==8978&&repeat_count==200&&mock_arg[1]==3);
TIMER0_IRQHandler();CHECK(mock_dac==0&&!mock_running[0]);EINT0_IRQHandler();CHECK(!mock_running[0]);
CHECK(sineValues[22]==500);
return 0;}
