...
(async()=>{
 ...
 for(const [name,query] of [['algorithms/index.html','quicksort'],['patterns/index.html','fifth argument']]){
   ...
   await page.locator('[data-filter-item]:visible [data-scope-match]:visible').first().waitFor();
 ...
 await page.goto(url('exams/2023-02-07-q2.html'));const q2tags=await page.locator('[data-exam-search-metadata]').textContent();check(q2tags.includes('EINT')&&!q2tags.includes('Timer IRQ'),'Raw button capture tags do not invent a timer IRQ');
