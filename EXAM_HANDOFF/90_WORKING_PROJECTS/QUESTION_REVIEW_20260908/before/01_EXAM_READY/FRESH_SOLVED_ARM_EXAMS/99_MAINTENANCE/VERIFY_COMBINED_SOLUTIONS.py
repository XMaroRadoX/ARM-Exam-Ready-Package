...
def main() -> int:
    ...
    if declared_api != set(API_DOCS):
        issues.append(
            f"header/documentation mismatch: missing {sorted(set(API_DOCS)-declared_api)}, undocumented {sorted(declared_api-set(API_DOCS))}"
        )
    ...
    print(
        ...
        f"Assembly={len(assembly_files)} API used={len(used_api)}/{len(declared_api)}"
    )
    return 0
