    ...
    for row in exams:
        ...
        for number, q in enumerate(exam_questions, 1):
            ...
            if question_asm_path:
                ...
                write(question_asm_path, source_code(specific if specific and specific.exists() else asm_path))
            ...
            if question_notes and question_notes.exists():
                note_html = markdown_fragment(source_code(question_notes), question_notes, question_destination)
                question_body = question_body.replace('<section class="section-block">' + code_sections, '<section class="section-block">' + note_html + '</section><section class="section-block">' + code_sections, 1)
                question_body = re.sub(r'<section class="section-block"><h2>Why the important lines are there</h2>.*?</section>', '', question_body, flags=re.S)
