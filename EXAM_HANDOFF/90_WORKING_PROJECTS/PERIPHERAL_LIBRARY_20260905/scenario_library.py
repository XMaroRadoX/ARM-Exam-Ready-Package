...
def historical_specs(c):
    ...
    for exam in exams:
        ...
        result[key]=dict(code=text,assembly=assembly,title=exam['project'].replace('_',' '),purpose=' '.join(q['requirement_summary'] for q in qs),
          actions='Follow the exact input sequence and expected result in the linked original question and reviewed mapping.',
          resources=[q['question_id']+': '+q['function_or_handler']+'; '+q['argument_mapping']+'; '+q['constants'] for q in qs],
...
def classify(c,items):
    ...
    for row in coverage:
        ...
        if target.exists():
            ...
            content='<section id="complete-project-method"><h2>Complete projects and worked method</h2>'+content+'</section>'
...
def rebase(text,old,new,moves):
    ...
    def change(m):
        ...
        value=quote(value,safe='/.-_')+('?' +url.query if url.query else '')+('#'+url.fragment if url.fragment else '')
...
def render_scenario(c,key,spec):
    ...
    return dict(id=key,title=spec['title'],route=c['rel'](dest,c['PORTAL']/'search.html'),path=folder.relative_to(ROOT).as_posix(),files=hashes,features=spec['features'],history=spec['history'],questions=spec['questions'],group=spec['group'],mode=spec.get('mode'),clock=spec.get('clock'),resources=spec['resources'],expected=spec['actions'],source=spec['source'])
...
def finalize(c,items):
    ...
    specs={**practice_specs(),**historical_specs(c)}
    records=[]
    ...
    for r in records:
        if r['questions']:body+='<li>'+c['href'](c['PORTAL']/r['route'],dest,', '.join(r['questions'])+' — '+r['title'])+'</li>'
