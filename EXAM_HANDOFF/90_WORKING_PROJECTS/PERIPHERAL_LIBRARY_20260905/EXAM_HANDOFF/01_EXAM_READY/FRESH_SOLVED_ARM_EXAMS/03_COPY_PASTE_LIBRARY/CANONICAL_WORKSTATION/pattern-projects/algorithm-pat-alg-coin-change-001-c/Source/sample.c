...
static int pattern_core_vector(void) {
  uint16_t c[] = {1, 3};
  uint32_t d[5];
  return algorithm_algorithm_pat_alg_coin_change_001_c(c, 2, 4, d) == 2;
}
static int pattern_edge_vectors(void) {
  uint16_t c[] = {2};
  uint32_t d[4];
  return algorithm_algorithm_pat_alg_coin_change_001_c(c, 1, 0, d) == 0 &&
         algorithm_algorithm_pat_alg_coin_change_001_c(c, 1, 3, d) == UINT32_MAX;
}
