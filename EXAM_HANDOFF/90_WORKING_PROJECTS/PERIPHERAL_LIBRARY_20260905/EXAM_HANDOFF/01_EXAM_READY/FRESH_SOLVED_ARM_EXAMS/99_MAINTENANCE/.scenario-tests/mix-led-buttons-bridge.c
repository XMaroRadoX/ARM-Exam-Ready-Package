...
#define MODE 0
#define CONTROL_CLOCK 0
#define DEBOUNCE_STEPS 1
#define POLL_BUTTONS 1
#define HAS_LED 1
...
void controller_step(uint32_t buttons,uint32_t joy,uint32_t valid,uint32_t adc) {
  ...
  if(valid) app.value=adc>>4;
  if(!HAS_ADC && !HAS_JOYSTICK && !HAS_BUTTONS && app.ticks%50u==0u) app.value=(app.value+1u)&255u;
#endif
