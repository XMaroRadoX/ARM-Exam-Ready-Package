...
Likely wording: fill connected grid cells.

## C contract and variants

Primary export: `uint32_t pat_alg_flood_fill_001(uint8_t *g, uint32_t rows, uint32_t cols, uint32_t start, uint8_t oldv, uint8_t newv, uint32_t *q);`
...
- [Readable C](../../../../../02%20-%20Code%20Recipes/11%20-%20Maximum%20Algorithm%20Reference/PAT-ALG-FLOOD-FILL-001/c/reference.c)
- [ARMASM implementation](../../../../../02%20-%20Code%20Recipes/11%20-%20Maximum%20Algorithm%20Reference/PAT-ALG-FLOOD-FILL-001/arm/implementation.s)
...
|---|---|
| `R0` | `g` — `uint8_t *g` |
| `R1` | `rows` — `uint32_t rows` |
| `R2` | `cols` — `uint32_t cols` |
| `R3` | `start` — `uint32_t start` |
| `caller [SP, #0]` | `oldv` — `uint8_t oldv` |
| `caller [SP, #4]` | `newv` — `uint8_t newv` |
| `caller [SP, #8]` | `q` — `uint32_t *q` |
