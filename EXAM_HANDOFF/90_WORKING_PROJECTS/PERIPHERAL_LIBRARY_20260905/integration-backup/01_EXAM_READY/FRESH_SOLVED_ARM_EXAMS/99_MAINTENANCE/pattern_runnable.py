...
def write_project(key,spec,clean,reuse=False):
    project=OUT/key
    if not reuse:
        shutil.copytree(TEMPLATE,project,dirs_exist_ok=True,ignore=shutil.ignore_patterns('Objects','Listings','*.uvguix.*','*.bak','*.log','*.axf','*.htm'))
    ...
    if reuse:
        ...
        return files
    for name,code in files.items():
        text=clean(code,key);(project/name).write_text(text,encoding='utf-8')
