...
void board_init(void)
{
  ...
  led_value = 0u;
#endif
  /* Peripherals are intentionally opt-in. An exam that only needs a timer
   * must not silently enable button, RIT, ADC, DAC or joystick resources. */
...
/* Callback helpers need the template's matching IRQ handler. If an exam owns
 * the exact vector, the raw handler is the only owner and callbacks must not
 * pretend that they will run. */
 ...
 * These wrappers validate signed exam inputs before converting them to the
 * precise unsigned driver API.  Existing APIs are deliberately untouched. */
                                                 ...
                                                 timer_callback_t callback)
{
  ...
  /* This beginner helper intentionally owns MR0 and makes it periodic. The
   * clock divider and prescaler remain whatever the caller selected. */
