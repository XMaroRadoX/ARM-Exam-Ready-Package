...
void LCD_draw_maze(int row, int column, uint8_t maze[row][column], int zoom)
{
	int i, j;
	for (i=0; i<row; i++)
	{
		...
		{
			if (maze[i][j] == 255)
				draw_border(j*zoom, i*zoom, zoom, zoom);
			else
			{
				int width = zoom / 8;			
				/* if bit 1 is 1, draw a wall at the right */
				if ((maze[i][j] & (1 << 1)) == 0)
					draw_border((j + 1) * zoom - width, i*zoom, zoom, width);
				/* if bit 2 is 1, draw a wall at the bottom */
				if ((maze[i][j] & (1 << 2)) == 0)
					draw_border(j*zoom, (i + 1) * zoom - width, width, zoom);
				/* if bit 3 is 1, draw a wall at the right */
				if ((maze[i][j] & (1 << 3)) == 0)
					draw_border(j * zoom, i*zoom, zoom, width);
				/* if bit 4 is 1, draw a wall at the top */
				if ((maze[i][j] & (1 << 4)) == 0)
					draw_border(j*zoom, i * zoom, width, zoom);
			}
		}
	}
}
	
void draw_border(int x, int y, int length, int height)
{
	int i,j;
	for (i=0; i<length; i++)
		for (j=0; j<height; j++)
...
void draw_position(int x, int y, int zoom, int color)
{
	int i,j;
	int width = 4;
	for (i= -width; i<width; i++)
		for (j= -width; j<width; j++)
			LCD_SetPoint(x * zoom + zoom/2 + j, y * zoom + zoom/2 + i, color);
}	
