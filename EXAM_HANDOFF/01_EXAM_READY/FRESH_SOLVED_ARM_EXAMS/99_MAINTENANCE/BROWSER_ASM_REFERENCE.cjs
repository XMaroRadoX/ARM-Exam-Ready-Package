...
(async()=>{
 ...
 try{
  ...
  const fill=async value=>{await page.locator('#asm-filter').fill(value);await page.waitForTimeout(100);};
  ...
  await fill('fifth argument');check(await page.locator('#fifth-argument').isVisible(),'Plain language filter');
  await fill('unsigned less than');check(await page.locator('#blo').isVisible(),'Unsigned comparison phrase');
  await fill('BICLS');check(await page.locator('#and').isVisible(),'Observed conditional mnemonic');
  await fill('not-a-real-instruction');check(await page.locator('[data-asm-entry]:visible').count()===0,'No results');
  ...
  await page.locator('#asm-clear').click();await fill('LDRB');
