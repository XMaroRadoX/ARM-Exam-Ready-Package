...
Test the current held-state mask for a diagonal; the two controls may become pressed in different samples, so requiring both pressed-edge bits together can miss it. A held-state test remains true while held; implement one-action or repeat behavior separately as required by the paper.
...
# Keep the practical additions beside their existing peripheral lesson.
for _lesson in BOARD:
    if _lesson['id']=='board-timers':
        _lesson['explanation']+='\n\nFor multiple matches, first configure and stop the timer, then call exam_timer_config_match for MR0-MR3. EXAM_MATCH_INTERRUPT, EXAM_MATCH_RESET and EXAM_MATCH_STOP combine independently. Start explicitly. exam_timer_set_prescaler clears PC and preserves TC; exam_timer_set_clock_divider accepts1,2,4,8. Both require an initialized, powered, stopped timer. Basic ticks/ms/hz configuration resets PR to0, so apply advanced settings afterward. Acknowledge once per handler, then test that saved mask with exam_timer_match_happened or exam_timer_capture_happened. The complete Timer setup and ownership pattern demonstrates all calls and a real P1.26 CAP0.0 setup.'
    if _lesson['id']=='board-joystick':
        _lesson['explanation']+='\n\nexam_joystick_released_edges(previous,current) returns directions that were held and are now released. It uses the same argument order as pressed_edges, masks to five valid bits, and stores no state. Save current as previous after both comparisons. The Joystick event state machine pattern includes a complete press/hold/release project without automatic repeat.'
