 ...
 */
int toggle_bit_u32(uint32_t value, uint32_t index,
                 uint32_t *result_out) {
...
#include <stddef.h>
#include <limits.h>
int toggle_bit_u32(uint32_t value, uint32_t index, uint32_t *result_out);
int test_main(void) {
uint32_t out=77;CHECK(toggle_bit_u32(8u,31,&out)&&out==0x80000008u);
out=77;CHECK(!toggle_bit_u32(8,32,&out)&&out==77);CHECK(!toggle_bit_u32(8,3,0));
return 0;
}
