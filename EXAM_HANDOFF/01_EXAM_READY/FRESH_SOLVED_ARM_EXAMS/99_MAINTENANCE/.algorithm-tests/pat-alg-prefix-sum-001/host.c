...
int pat_alg_prefix_sum_001(const int32_t *values, uint32_t count,
                           int64_t *prefix) {
  uint32_t index;

  if ((prefix == NULL) || ((values == NULL) && (count != 0u)) || count >= UINT32_MAX/8u) {
    return 0;
  }
  prefix[0] = 0;
...
int pat_alg_prefix_sum_001_range(const int64_t *prefix, uint32_t count,
