 ...
 * Contract: Return the signed maximum in one row. Require dimensions 1..256, a valid selected index, and valid pointers. Invalid input writes nothing.
 * Method:
 ...
 */
int matrix_row_maximum_i32(const int32_t *matrix, uint32_t rows,
...
CHECK(matrix_row_maximum_i32(a,2,3,1,&out)&&out==9);
out=77;CHECK(!matrix_row_maximum_i32(a,2,3,3,&out)&&out==77);
CHECK(!matrix_row_maximum_i32(a,2,3,0,0));
return 0;
}
