...
static int pattern_core_vector(void) {
  int32_t a[] = {1, -2, 4};
  int64_t p[4];
  algorithm_constant_time_range_sums(a, 3, p);
  return p[0] == 0 && p[1] == 1 && p[2] == -1 && p[3] == 3;
}
static int pattern_edge_vectors(void) {
  int32_t a[] = {1, -2, 4};
  int64_t p[4], s = 0;
  return algorithm_constant_time_range_sums(NULL, 0, p) &&
         algorithm_constant_time_range_sums(a, 3, p) &&
         algorithm_constant_time_range_sums_range(p, 3, 1, 3, &s) && s == 2 &&
         !algorithm_constant_time_range_sums_range(p, 3, 3, 2, &s);
}
