 ...
 * Contract: Return the signed minimum in one column. Require dimensions 1..256, a valid
 ...
 */
int matrix_column_minimum_i32(const int32_t *matrix, uint32_t rows, uint32_t columns,
...
int test_main(void) {
  ...
  out = 77;
  CHECK(!matrix_column_minimum_i32(a, 2, 3, 3, &out) && out == 77);
