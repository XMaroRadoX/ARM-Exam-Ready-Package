 ...
 * Contract: Count cells equal to target in a matrix with dimensions at most 256. A zero
 * dimension returns zero and permits a null matrix pointer. Other invalid input returns
 * zero. Method:
...
uint32_t matrix_count_matches_i32(const int32_t *matrix, uint32_t rows,
  ...
  if (rows > 256 || columns > 256 || (!matrix && rows && columns))
    return 0;
  uint32_t matches = 0;
