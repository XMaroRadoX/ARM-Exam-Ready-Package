...
static uint32_t pat_alg_bitfield_001_mask(uint32_t width) {
  if (width == 0u) {
    return 0u;
  }
...
uint32_t pat_alg_bitfield_001(uint32_t word, uint32_t value, uint32_t shift,
                              uint32_t width) {
  uint32_t field_mask;
  ...
  field_mask = pat_alg_bitfield_001_mask(width) << shift;
  return (word & ~field_mask) | ((value << shift) & field_mask);
}

uint32_t pat_alg_bitfield_001_extract(uint32_t word, uint32_t shift,
                                      uint32_t width) {
  if ((width == 0u) || (shift >= 32u) || (width > 32u - shift)) {
    return 0u;
  }
  return (word >> shift) & pat_alg_bitfield_001_mask(width);
}
