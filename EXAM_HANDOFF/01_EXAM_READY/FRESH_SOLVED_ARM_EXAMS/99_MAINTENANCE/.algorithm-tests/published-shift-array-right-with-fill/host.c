 ...
 */
int array_shift_right(int32_t *values, uint32_t count, uint32_t amount,
                      int32_t fill_value) {
  if (!values && count)
    return 0;
  if (amount > count)
...
int test_main(void) {
  ...
  CHECK(array_shift_right(a, 4, 2, -1));
  CHECK(a[0] == -1 && a[1] == -1 && a[2] == 1 && a[3] == 2 && a[4] == 77);
  CHECK(array_shift_right(a, 4, 9, 5) && a[0] == 5 && a[3] == 5);
  CHECK(array_shift_right(0, 0, 1, 5));
  CHECK(!array_shift_right(0, 1, 1, 5));
  return 0;
}
