...
#include <stddef.h>
#include <limits.h>
/* Exam prompt: Find the last occurrence of a character
 ...
 */
int32_t string_last_occurrence(const uint8_t *text, uint32_t capacity,
                         uint8_t target) {
  ...
  for (uint32_t i=0;i<length;++i)
    if (text[i]==target) { found=(int32_t)i; }
  return found;
}
...
int test_main(void) {
uint8_t a[]="BANANA",bad[]={'A','A'};CHECK(string_last_occurrence(a,7,'A')==5);
CHECK(string_last_occurrence(a,7,'Z')==-1);CHECK(string_last_occurrence(bad,2,'A')==-2);
