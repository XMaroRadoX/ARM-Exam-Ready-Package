...
void algorithm_bubble_sort_with_early_termination(int32_t *a, uint32_t n) {
  uint32_t i;
  if (!a)
    return;
  if (!a)
...
static int pattern_core_vector(void) {
  int32_t a[] = {3, -1, 2};
  algorithm_bubble_sort_with_early_termination(a, 3);
  return a[0] == -1 && a[1] == 2 && a[2] == 3;
}
static int pattern_edge_vectors(void) {
  int32_t a[] = {1, 2, 3}, b[] = {2, 2, 1};
  algorithm_bubble_sort_with_early_termination(a, 3);
  algorithm_bubble_sort_with_early_termination(b, 3);
