 ...
 * Contract: Swap two elements only when both indexes are in range and storage is valid.
 * Equal indexes are a successful no-op. Invalid input returns 0 without writing.
 * Method:
...
int array_swap_indexes(int32_t *values, uint32_t count, uint32_t first,
                       uint32_t second) {
  if (!values || first >= count || second >= count)
    return 0;
