 ...
 * Contract: Return the signed matrix extremum. Require dimensions 1..256 and valid pointers. Invalid or empty input returns 0 without writing.
 * Method:
 * 1. Validate nonempty dimensions.
 * 2. Seed from the first cell.
 * 3. Scan remaining row-major cells using signed comparisons.
 */
int matrix_maximum_i32(const int32_t *matrix, uint32_t rows,
  ...
  for(uint32_t i=1;i<count;++i) if(matrix[i]>best) best=matrix[i];
...
#include <stddef.h>
#include <limits.h>
int matrix_maximum_i32(const int32_t *matrix, uint32_t rows, uint32_t columns, int32_t *value_out);
int test_main(void) {
int32_t a[]={1,-6,3,4,9,2},out=77;
CHECK(matrix_maximum_i32(a,2,3,&out)&&out==9);
out=77;CHECK(!matrix_maximum_i32(a,0,3,&out)&&out==77);
