...
EXPECTED_COUNTS = {"exams": 23, "questions": 48, "patterns": 85, "api": len(API_DOCS), "algorithms": 169, "peripheral_lessons": 14}
...
def verify() -> list[str]:
    ...
    if template_directories != ["Official Combined Exam API"]:
        errors.append(f"Starting Templates must contain only Official Combined Exam API; found {template_directories}")
    ...
    actual_counts = {
        ...
        "algorithms": len(list((PORTAL / "algorithms").glob("*.html"))) - 1,
    ...
    inventory=json.loads((MAINTENANCE / 'ALGORITHM_INVENTORY.json').read_text()) if destinations else []
    ...
    expected_pages = {"exams": 72, "patterns": 86, "api": len(API_DOCS) + 1, "algorithms": 170, "guides": 24, "asm": 1}
    if destinations:
        expected_pages.update(patterns=expected_counts['patterns']+1,algorithms=expected_counts['algorithms']+1,guides=24+sum(r['section']=='Guides' for r in destinations.values()),asm=1+sum(r['section']=='ASM Reference' for r in destinations.values()))
    ...
    gap_report = API_HEADER.parents[2] / "API_GAP_REPORT.md"
    ...
    quick_reference = (API_HEADER.parents[2] / "EXAM_API_QUICK_REFERENCE.md").read_text(encoding="utf-8", errors="replace")
    ...
    if len(algorithm_sources) != 169: errors.append(f"Expected 169 algorithm references; found {len(algorithm_sources)}")
...
def main() -> int:
    ...
    print(f"Coverage: 23 exams, 48 questions, 85 patterns, {len(API_DOCS)} API calls, 169 algorithm entries, 14 peripheral lessons, 9 goal guides")
